    ...
    <form method="POST" action="{{ route('admin.skills.update', $skill) }}" class="space-y-6">
        ...
        <div class="bg-gray-800 rounded-xl border border-gray-700 p-6 space-y-5">
            ...
            window.addEventListener('load', function() {
                const existingUrl = "{{ old('icon', $skill->icon ?? '') }}";
                if (existingUrl) {
                    document.getElementById('preview-img').src = existingUrl;
            ...
            {{-- Level --}}
            <div x-data="{ level: {{ old('level', $skill->level) }} }">
            ...
            <div>
                ...
                <input type="number" id="order_column" name="order_column" value="{{ old('order_column', $skill->order_column) }}"
