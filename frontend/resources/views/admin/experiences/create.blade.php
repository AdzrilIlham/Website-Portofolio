...
<div class="max-w-2xl">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-white">Create Experience</h2>
        <a href="{{ route('admin.experiences.index') }}" class="text-gray-400 hover:text-white text-sm">
            &larr; Back to Experiences
        </a>
    </div>

    <form method="POST" action="{{ route('admin.experiences.store') }}" class="space-y-6" x-data="{ isCurrent: {{ old('is_current') ? 'true' : 'false' }} }">
        @csrf
        ...
        <div class="bg-gray-800 rounded-xl border border-gray-700 p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" id="company_or_institution" name="company_or_institution" value="{{ old('company_or_institution') }}"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                          ...
                          placeholder="Describe your role and achievements">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1.5 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
