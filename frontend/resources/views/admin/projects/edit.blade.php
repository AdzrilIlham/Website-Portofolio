...
<div class="max-w-2xl">
    ...
    <form method="POST" action="{{ route('admin.projects.update', $project) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-gray-800 rounded-xl border border-gray-700 p-6 space-y-5">
            ...
            {{-- Tech Stack --}}
            @php
                $techStackString = is_array($project->tech_stack)
                    ? implode(', ', $project->tech_stack)
                    : ($project->tech_stack ?? '');
            @endphp
            <div x-data="{ tags: '{{ $techStackString ? addslashes($techStackString) : '' }}'.split(',').map(s => s.trim()).filter(Boolean), input: '' }">
                <label class="block text-sm font-medium text-gray-300 mb-1.5">Tech Stack (comma separated)</label>
            ...
            {{-- Image --}}
            <div x-data="{ preview: '{{ $project->image ? asset('storage/' . $project->image) : '' }}' }">
            ...
            <div>
                ...
                <input type="number" id="order_column" name="order_column" value="{{ old('order_column', $project->order_column) }}"
